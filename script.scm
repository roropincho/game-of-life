...
(define (makeId x y)
		(let ((txtX (number->string x))
			 ...
			 (let ((id (string-append "x"
			 	  ...
			 	  (begin
			 	  	#|
			 	  	(console.log (string-append "generated :"
			 	  								id
			 	  								" with values :"))
			 	  	(console.log x)
			 	  	(console.log y)
...
(define gameOfLife
		(lambda ()
				(define (transferRow i)
						(if (< i nbRow)
							(begin
								; (console.log (vector-ref around i))
								; (console.log (vector-ref oldAround i))
										 	  	...
										 	  	(vector-set! (vector-ref around indY) indX
	  										 											(+ oldNb
	  										 										 	   (if wasAlive
	  										 										 		   ...
	  										 										 		   1)))
#|
										 	  	(console.log (string-append "from "
										 	  								(number->string oldNb)
										 	  								" to "
										 	  								(number->string (vector-ref (vector-ref around indY) indX))
										 	  								" with difs "
										 	  								(number->string difCol)
										 	  								" and "
										 	  								(number->string difRow)
										 	  								" arround "
										 	  								(number->string x)
										 	  								", "
										 	  								(number->string y)))
							...
							(begin
								(if (not (and (equal? difCol 0)
									...
									(let ((indX (modulo (+ x difCol nbCol) nbCol))
										  (indY (modulo (+ y difRow nbRow) nbRow)))
#|
										(console.log (string-append "around (" (number->string x) ", " (number->string y) ") : x " (number->string indX) " y " (number->string indY)))
										 |#
										 (vector-set! (vector-ref around indY) indX
